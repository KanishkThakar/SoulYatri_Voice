...
- [x] 1. Set up the roadmap tooling package and testing framework
  - [x] 1.1 Create the `roadmap/` package skeleton and test configuration
    ...
    - _Requirements: 12.7_
  - [x] 1.2 Add a configuration smoke test
...
- [x] 2. Implement phase, track, and gate data models
  - [x] 2.1 Implement `Track`, `Phase`, `GateCriterion`, and `Gate` dataclasses in `roadmap/models/phases.py`
  ...
  - [x] 2.2 Write unit tests for phase/gate construction and field validation
    - Cover operator set, required guide-section presence, and compute-footprint fields
    - _Requirements: 1.1, 1.4_

- [x] 3. Implement gate-result and model-selection data models
  - [x] 3.1 Implement `CriterionResult`, `GateResult`, `CandidateScores`, `SelectionOutcome` plus `SELECTION_WEIGHTS` and `SELECTION_MINIMUMS` in `roadmap/models/selection.py`
  ...
  - [x] 3.2 Write unit tests asserting weights sum to 100 and every criterion has a minimum
    - _Requirements: 2.3_

- [x] 4. Implement harness, emotion, safety, licensing, and normalization data models
  - [x] 4.1 Implement remaining dataclasses in `roadmap/models/runtime.py`
  ...
  - [x] 4.2 Write unit tests for V/A/D clamping and the consent-record validity helper
    - _Requirements: 6.1, 10.1_

- [x] 5. Implement the Roadmap aggregate and phase-structure invariant
  - [x] 5.1 Implement `Roadmap` in `roadmap/roadmap_model.py`
  ...
  - [x] 5.2 Write property test for the phase-structure invariant
    - **Property 1: Phase-structure invariant**
    - **Validates: Requirements 1.1, 1.6, 9.1**

- [x] 6. Implement the Licensing Register
  - [x] 6.1 Implement `LicensingRegister` in `roadmap/licensing.py`
  ...
  - [x] 6.2 Write property test for the licensing register and redistribution exclusion
    - **Property 15: Licensing register and redistribution exclusion**
    - **Validates: Requirements 11.1, 11.2, 11.3, 11.6, 11.8**

- [x] 7. Implement the Gate Evaluator
  - [x] 7.1 Implement `GateEvaluator` in `roadmap/gate_evaluator.py`
  ...
  - [x] 7.2 Write property test for gate evaluation, blocking, remediation, and licensing block
    - **Property 2: Gate evaluation, blocking, remediation, and licensing block**
    - **Validates: Requirements 1.4, 1.5, 8.4, 8.5, 11.5, 11.7**
  - [x] 7.3 Write unit tests for boundary cases
...
- [x] 8. Implement the Selection Scorer
  - [x] 8.1 Implement `SelectionScorer` in `roadmap/selection_scorer.py`
  ...
  - [x] 8.2 Write property test for Speech-Native Core selection
    - **Property 3: Speech-Native Core selection**
    - **Validates: Requirements 2.4, 2.7, 2.9**
  - [x] 8.3 Write unit test for the selection tie-break behavior
    - _Requirements: 2.9_

- [x] 9. Checkpoint — core data models, registries, gate, and selection logic
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. Implement the Evaluation Harness (comparability and record completeness)
  - [x] 10.1 Implement the mocked `EvaluationHarness` in `roadmap/evaluation_harness.py`
  ...
  - [x] 10.2 Write property test for harness comparability and record completeness
    - **Property 4: Evaluation harness comparability and record completeness**
    - **Validates: Requirements 4.1, 4.2, 4.3**

- [x] 11. Add exclusion handling to the Evaluation Harness
  - [x] 11.1 Extend `EvaluationHarness` with per-candidate exclusion recording in `roadmap/evaluation_harness.py`
  ...
  - [x] 11.2 Write property test for exclusion recording without abort
    - **Property 5: Exclusion recording without abort**
    - **Validates: Requirements 4.5, 11.4**

- [x] 12. Add reproducibility support to the Evaluation Harness
  - [x] 12.1 Extend `EvaluationHarness` with reproducible re-runs in `roadmap/evaluation_harness.py`
    ...
    - _Requirements: 4.4_
  - [x] 12.2 Write property test for harness reproducibility within tolerance
    - **Property 6: Harness reproducibility within tolerance**
    - **Validates: Requirements 4.4**

- [x] 13. Implement Hinglish transliteration normalization
  - [x] 13.1 Implement `HinglishDataEngine.normalize(text)` in `roadmap/hinglish.py`
  ...
  - [x] 13.2 Write property test for transliteration determinism and unmapped-token handling
    - **Property 8: Transliteration determinism and unmapped-token handling**
    - **Validates: Requirements 5.5, 5.8**

- [x] 14. Implement Hinglish code-switch quality scoring
  - [x] 14.1 Implement `HinglishDataEngine.score(input, output)` and the benchmark pass rule in `roadmap/hinglish.py`
  ...
  - [x] 14.2 Write property test for code-switch quality scoring and pass threshold
    - **Property 7: Code-switch quality scoring and pass threshold**
    - **Validates: Requirements 5.4, 5.6**

- [x] 15. Implement the Emotion/Persona Controller
  - [x] 15.1 Implement `EmotionPersonaController.select_conditioning(features)` in `roadmap/emotion_persona.py`
  ...
  - [x] 15.2 Write property test for emotion/persona conditioning selection
    - **Property 9: Emotion/persona conditioning selection**
    - **Validates: Requirements 6.1, 6.3, 6.5, 6.7**
  - [x] 15.3 Write unit test for the distress confidence boundary
    - A distress label at exactly confidence 0.50 selects `EMPATHETIC`
    - _Requirements: 6.5_

- [x] 16. Checkpoint — evaluation harness and Hinglish/emotion logic
  - Ensure all tests pass, ask the user if questions arise.

- [x] 17. Implement Duplex Manager barge-in timing
  - [x] 17.1 Implement barge-in decision logic in `roadmap/duplex_manager.py`
  ...
  - [x] 17.2 Write property test for barge-in suppression and listening transition
    - **Property 10: Barge-in suppression and listening transition**
    - **Validates: Requirements 7.4, 7.5**
  - [x] 17.3 Write unit test for barge-in from an intermediate state
    - A barge-in from a `THINKING`/processing state reaches `LISTENING` within budget
    - _Requirements: 7.5_

- [x] 18. Implement Duplex Manager latency-only fallback
  - [x] 18.1 Implement latency-only fallback discrimination in `roadmap/duplex_manager.py`
    ...
    - _Requirements: 3.5_
  - [x] 18.2 Write property test for latency-only fallback discrimination
    - **Property 11: Latency-only fallback discrimination**
    - **Validates: Requirements 3.5**

- [x] 19. Implement Safety Guard watermarking
  - [x] 19.1 Implement watermark application in `roadmap/safety_guard.py`
    ...
    - _Requirements: 10.2_
  - [x] 19.2 Write property test for watermark on every output path
    - **Property 12: Watermark on every output path**
    - **Validates: Requirements 10.2**

- [x] 20. Implement Safety Guard consent gating
  - [x] 20.1 Implement consent gating and the consent log in `roadmap/safety_guard.py`
  ...
  - [x] 20.2 Write property test for consent gating and consent-log completeness
    - **Property 13: Consent gating and consent-log completeness**
    - **Validates: Requirements 10.1, 10.3, 10.6**

- [x] 21. Implement Safety Guard crisis handling
  - [x] 21.1 Implement crisis handling in `roadmap/safety_guard.py`
  ...
  - [x] 21.2 Write property test for crisis handling
    - **Property 14: Crisis handling**
    - **Validates: Requirements 10.4, 10.5**

- [x] 22. Implement the Budget & Compute Planner
  - [x] 22.1 Implement `BudgetComputePlanner` in `roadmap/budget_planner.py`
  ...
  - [x] 22.2 Write unit/property test for `select_scope` on insufficient, zero, and invalid/missing compute
    - Returned footprint never exceeds the confirmed-available compute
    - _Requirements: 9.5_

- [x] 23. Author the narrative roadmap planning artifact (`roadmap/ROADMAP.md`)
  - [x] 23.1 Author the phase/track/gate structure and traceability sections
  ...
  - [x] 23.2 Author the model-adoption, evolution, moat, emotion, duplex/latency, and benchmark sections
  ...
  - [x] 23.3 Author the budget/compute, safety-scope, and scope/relationship sections
...
- [x] 24. Write structural/example tests for the narrative roadmap content
  - [x] 24.1 Write structural tests for track/gate structure and scope statements in `roadmap/tests/test_structural_structure.py`
  ...
  - [x] 24.2 Write structural tests for enumerations and designations in `roadmap/tests/test_structural_enumerations.py`
  ...
  - [x] 24.3 Write structural tests for migration/reuse statements, targets, and policies in `roadmap/tests/test_structural_targets.py`
  ...
  - [x] 24.4 Write the PDF-citation existence test in `roadmap/tests/test_pdf_citations.py`
    - Assert every `docs/` file name referenced in `ROADMAP.md` exists in the actual `docs/` directory listing
    - _Requirements: 12.6_

- [x] 25. Integrate the decision-logic tooling
  - [x] 25.1 Wire the components into a cohesive package API in `roadmap/__init__.py`
  ...
  - [x] 25.2 Write the end-to-end integration test in `roadmap/tests/test_integration_harness.py`
...
- [x] 26. Final checkpoint — ensure all property and structural tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 27. Implement serialization for the planning objects (JSON/YAML)
  - [x] 27.1 Implement `to_dict`/`from_dict` and JSON/YAML codecs in `roadmap/serialization.py`
  ...
  - [x] 27.2 Write a property-based round-trip test in `roadmap/tests/test_serialization_roundtrip.py`
  ...
  - [x] 27.3 Write unit tests for serialization edge cases in `roadmap/tests/test_serialization_formats.py`
...
- [x] 28. Add a JSON Schema and schema-validation layer for the planning models
  - [x] 28.1 Author JSON Schemas and a validator in `roadmap/schemas/` and `roadmap/schema.py`
  ...
  - [x] 28.2 Write unit tests for schema validation in `roadmap/tests/test_schema.py`
  ...
  - [x] 28.3 Write a property test that serialized models satisfy their schema in `roadmap/tests/test_schema_roundtrip.py`
...
- [x] 29. Load the roadmap from a structured data file and round-trip to `ROADMAP.md`
  - [x] 29.1 Author `roadmap/data/roadmap_data.yaml` and a loader in `roadmap/roadmap_io.py`
  ...
  - [x] 29.2 Implement `ROADMAP.md` round-trip in `roadmap/roadmap_io.py`
  ...
  - [x] 29.3 Write round-trip and structural tests in `roadmap/tests/test_roadmap_io.py`
    - Assert `load_roadmap` produces a valid aggregate and that `parse_markdown(render_markdown(r))` equals the loaded roadmap
    - _Requirements: 1.1, 1.6_

- [x] 30. Cross-check the `ROADMAP.md` narrative against the planning data objects
  - [x] 30.1 Implement a consistency checker in `roadmap/consistency.py`
  ...
  - [x] 30.2 Write tests for the consistency checker in `roadmap/tests/test_consistency.py`
    - A matching pair reports no divergence; an injected mismatch (changed label, dropped conflict row) is reported
    - _Requirements: 1.2, 1.7_

- [x] 31. Render a combined human-readable gate report
  - [x] 31.1 Create the reports package and `roadmap/reports/gate_report.py`
    ...
    - _Requirements: 1.4, 1.5, 8.4, 8.5, 11.5, 11.7_
  - [x] 31.2 Write unit tests for the gate report in `roadmap/tests/test_gate_report.py`
...
- [x] 32. Render a candidate-model evaluation report
  - [x] 32.1 Implement `roadmap/reports/evaluation_report.py`
  ...
  - [x] 32.2 Write unit tests for the evaluation report in `roadmap/tests/test_evaluation_report.py`
    - All non-excluded rows share the identical metric columns; excluded candidates appear with category + reason
    - _Requirements: 4.1, 4.5_

- [x] 33. Export the conflict register and licensing register
  - [x] 33.1 Implement `roadmap/reports/registers_report.py`
  ...
  - [x] 33.2 Write unit tests for the register exports in `roadmap/tests/test_registers_report.py`
...
- [x] 34. Externalize the Hinglish transliteration mapping and report coverage
  - [x] 34.1 Author `roadmap/data/hinglish_map.json` and load it in `roadmap/hinglish.py`
    ...
    - _Requirements: 5.5, 5.8_
  - [x] 34.2 Implement an unmapped-token coverage report in `roadmap/reports/hinglish_coverage.py`
    ...
    - _Requirements: 5.8_
  - [x] 34.3 Write tests for the mapping loader and coverage report in `roadmap/tests/test_hinglish_map.py`
...
- [x] 35. Render a latency-budget simulation report
  - [x] 35.1 Implement `roadmap/reports/latency_report.py`
  ...
  - [x] 35.2 Write unit tests for the latency report in `roadmap/tests/test_latency_report.py`
...
- [x] 36. Render a consent-log and watermark-compliance audit report
  - [x] 36.1 Implement `roadmap/reports/safety_audit.py`
  ...
  - [x] 36.2 Write unit tests for the safety audit in `roadmap/tests/test_safety_audit.py`
...
- [x] 37. Render a per-phase budget and compute summary
  - [x] 37.1 Implement `roadmap/reports/budget_report.py`
    ...
    - _Requirements: 9.1, 9.2, 9.3, 9.4, 9.5_
  - [x] 37.2 Write unit tests for the budget report in `roadmap/tests/test_budget_report.py`
...
- [x] 38. Generate the requirement-to-task-to-test traceability matrix
  - [x] 38.1 Implement a traceability generator in `roadmap/reports/traceability.py`
  ...
  - [x] 38.2 Write tests asserting full criterion coverage in `roadmap/tests/test_traceability.py`
    - The generated matrix maps every requirements criterion to at least one task; the uncovered-criteria list is empty
    - _Requirements: 1.6, 12.7_

- [x] 39. Generate a property-test coverage report
  - [x] 39.1 Implement a property-coverage scanner in `roadmap/reports/property_coverage.py`
    ...
    - _Requirements: 12.7_
  - [x] 39.2 Write a test asserting all 15 properties are present in `roadmap/tests/test_property_coverage.py`
    - Assert the scanner finds exactly Properties 1 through 15, each tagged once, with no gaps and no numbers beyond 15
    - _Requirements: 12.7_

- [x] 40. Generate API reference and per-component documentation pages
  - [x] 40.1 Implement a docstring/API-reference generator in `roadmap/docs_gen.py`
  ...
  - [x] 40.2 Author per-component documentation pages under `roadmap/docs/`
    ...
    - _Requirements: 12.7_
  - [x] 40.3 Write tests for the docs generator in `roadmap/tests/test_docs_gen.py`
...
- [x] 41. Add a CLI for the roadmap tooling
  - [x] 41.1 Implement `roadmap/cli.py` and register a console entry point
  ...
  - [x] 41.2 Write CLI smoke tests in `roadmap/tests/test_cli.py`
    - Each subcommand runs on fixture data, exits zero, and never invokes a GPU/training run
    - _Requirements: 4.6, 12.7_

- [x] 42. Author the roadmap package README
  - [x] 42.1 Write `roadmap/README.md`
    ...
    - _Requirements: 4.6, 12.7_
  - [x] 42.2 Write a README link/structural test in `roadmap/tests/test_readme.py`
    - Assert the README references each report module and CLI subcommand that exists in the package
    - _Requirements: 12.7_

- [x] 43. Add linting and type-checking for the roadmap package
  - [x] 43.1 Configure ruff and mypy scoped to `roadmap/` in `pyproject.toml`
    ...
    - _Requirements: 12.7_
  - [x] 43.2 Add a lint/type configuration smoke test in `roadmap/tests/test_tooling_config.py`
    - Assert the ruff and mypy configuration targets `roadmap/` and that `py.typed` is present
    - _Requirements: 12.7_

- [x] 44. Add a local test-runner and pre-commit configuration
  - [x] 44.1 Author a local test-runner script `scripts/run_roadmap_tests.ps1`
    ...
    - _Requirements: 4.6, 12.7_
  - [x] 44.2 Author a `.pre-commit-config.yaml` for the roadmap package
    ...
    - _Requirements: 4.6, 12.7_
  - [x] 44.3 Write a meta-test validating the runner configuration in `roadmap/tests/test_test_runner.py`
    - Assert the runner script and pre-commit config reference the `roadmap/tests` path and the Hypothesis profile
    - _Requirements: 4.6_

- [x] 45. Consolidate edge-case and regression test suites
  - [x] 45.1 Implement shared test fixtures and a regression-cases data file in `roadmap/tests/fixtures.py` and `roadmap/tests/regression_cases.json`
    ...
    - _Requirements: 12.7_
  - [x] 45.2 Write a consolidated edge-case suite in `roadmap/tests/test_edge_cases.py`
  ...
  - [x] 45.3 Write a regression suite in `roadmap/tests/test_regression.py`
...
- [x] 46. Final checkpoint — end-to-end validation and traceability completeness
